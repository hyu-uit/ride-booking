...
const StudentOfficeScreen = ({ navigation }) => {
  ...
  const subscribeToUserChanges = () => {
    ...
    const customerQuery = query(customerCollectionRef, where("status", "==", "pending"));
    const riderQuery = query(riderCollectionRef, where("status", "==", "pending"));
    ...
    const unsubscribeCustomer = onSnapshot(customerQuery, (querySnapshot) => {
      ...
      setUsers((prevUsers) => [...prevUsers.filter((user) => user.role !== "Customer"), ...updatedUsers]);
    });
    const unsubscribeRider = onSnapshot(riderQuery, (querySnapshot) => {
      ...
      setUsers((prevUsers) => [...prevUsers.filter((user) => user.role !== "Rider"), ...updatedUsers]);
    });
