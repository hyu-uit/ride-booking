import { Box, Button, Center, Divider, HStack, Image, Spinner, Text, VStack, View } from 'native-base'
...
const LocationCardFinder = () => {
  return (
    <View bgColor={'#0B0F2F'} w={'100%'} h={410} borderTopRadius={20} shadow={3} position={'absolute'} bottom={0} padding={'30px 25px 0 25px'}>
      <VStack space={3}>
        <Text fontSize={SIZES.h3} bold color={'white'}>Finding your driver</Text>
        <HStack w={'100%'}>
          <VStack space={2}>
            <VStack space={1}>
        ...
        <Text fontSize={SIZES.h3} bold color={'white'}>Driver detail</Text>
        <Center h={'40px'}>
          <HStack space={2}>
            <Spinner size="lg" />
            <Text fontSize={SIZES.h4} bold color={'white'}>Finding your driver...</Text>
          </HStack>
        </Center>
        <HStack>
          <Button  bgColor={'#3D5AF8'} w={'100%'} borderRadius={'20px'}>
            <Text color={'white'} bold fontSize={SIZES.small}>Cancel</Text>
          </Button>
        </HStack>
      </VStack>
    </View>
